      ...
      INTEGER :: istat = 0 
        ...
        nchrgsr = 0 
        ALLOCATE ( snp(pchrgmx), STAT=istat)
        snp = 0 
        ALLOCATE ( aimp(pchrgmx), STAT=istat)
        aimp = 0 
        ALLOCATE ( bimp(pchrgmx), STAT=istat)
        bimp = 0 
        ALLOCATE ( cimp(pchrgmx), STAT=istat)
        cimp = 0 
        ALLOCATE ( dimp(pchrgmx), STAT=istat)
        dimp = 0 
        ALLOCATE ( ework(pchrgmx), STAT=istat)
        ework = 0 
        ALLOCATE ( fwork(pchrgmx), STAT=istat)
        fwork = 0                                                          
      if (istat .ne. 0) then 
         print *, 'Allocation Error : alloc_impcom   ' 
         stop
      end if
            
      return
      END SUBROUTINE alloc_impcom  
