      subroutine grap(imode,zz,xx,gradsq,dpsidx,dpsidz,gsval,psval,      &  
...
!
      ...
      vmat(2,ii) = .50_R8*(psi(i+1,j  ) - psi(i-1,j  )   )
      vmat(3,ii) = .50_R8*(psi(i  ,j+1) - psi(i  ,j-1)   )
      vmat(4,ii) = .25_R8*(psi(i+1,j+1) - psi(i-1,j+1)                   &  
...
!
      ...
      subroutine evalpr(zz,xx,prval)
      USE CLINAM
      USE SAPROP
      IMPLICIT NONE
      INTEGER, PARAMETER :: R8=SELECTED_REAL_KIND(12,100)
       REAL*8 zz,xx,prval, xp, zp
      INTEGER :: ival, jval
!============      
!
      jval  = (zz-zzero)/deez+2.5
      ival  = (xx-ccon )/deex+2.5
!
      if(jval .lt. 2) jval = 2
      if(jval .gt. nz) jval = nz
      if(ival .lt. 2) ival = 2
...
!
!...........................................................
      xp     = (xx-xary(ival))/deex + 0.5
      zp     = (zz-zary(jval))/deez + 0.5
!
!
      prval =                                                            &
     &                xp*zp*pr(ival+1,jval+1)                            &
     &         + xp*(1._R8-zp)*pr(ival+1,jval)                           &
     &         + zp*(1._R8-xp)*pr(ival,jval+1)                           &
     &    + (1._R8-xp)*(1._R8-zp)*pr(ival,jval)
!
      return
      end
