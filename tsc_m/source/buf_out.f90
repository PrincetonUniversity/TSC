      SUBROUTINE buf_out( lu, idrss, idrse )
      USE CLINAM
      USE SAPROP
      USE NONCOR
      USE SPECIE
...
!     SELECT CASE (trim(idrss)) 
     ...
     &                     gvolt0(:pngroup),                             &  
!cj  &                fsave1(:pnfeed,:pdelay),                           &  
     &         ((fsave1(i,k),i=1,pnfeed),k=1,pdelay),                    &  
     &                sumfeed(:pnfeed),savfeed(:pnfeed),                 &  
!cj  &                 xcon0(:ptpts,:10), zcon0(:ptpts,:10),             &  
     & ((xcon0(i,k),i=1,ptpts),k=1,10), ((zcon0(i,k),i=1,ptpts),k=1,10), &  
!cj  &   fadj(:pngroup,:pncoil),amats(:20,:20,:pdelay),                  &  
     &          ((fadj(i,k),i=1,pngroup),k=1,pncoil),                    &  
     &         (((amats(i,k,j),i=1,20),k=1,20),j=1,pdelay),              &  
!cj  &   bvecs(:20,:pdelay),shpsum(:20),shpdif(:20),shpdifo(:20),        &  
!cj  &   bvecs(:20,:pdelay),                                             &  
     &          ((bvecs(i,k),i=1,20),k=1,pdelay),                        &  
     &                      shpsum(:20),shpdif(:20),shpdifo(:20),        &  
...
!cj  &     aplostr(:pnthe,:ppsi),                                        &  
     ...
     &                fincwa
      ...
      print *, "Error in buf_out: match not found", trim(idrss)

      END SELECT

      return
      END SUBROUTINE buf_out
