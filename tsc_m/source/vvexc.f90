...
! idecl:  explicitize implicit INTEGER declarations:
      INTEGER jvvhi,jvvlo,kvvgrp,kmxx,i,n,j,iw,jw
!============
...
!============      
      INTEGER :: istat = 0 
      INTEGER, ALLOCATABLE, DIMENSION(:) :: ivvlo
      INTEGER, ALLOCATABLE, DIMENSION(:) :: ivvhi
!============      
      IF(.not.ALLOCATED(ivvlo)) ALLOCATE( ivvlo(pnz), STAT=istat)
      IF(.not.ALLOCATED(ivvhi)) ALLOCATE( ivvhi(pnz), STAT=istat)
!============      
      if (istat .ne. 0) stop 'Allocation Error : vvexc  ' 
!============      
!
...
!
 ...
 3011 format(" NOTE: Special logic assumes NSTX Vacuum vessel ")
...
!
        ...
        if (igroupw(n).gt.10 .and. igroupw(n).ne.17 .and. igroupw(n).ne.18)   then
        if (zwire(n).le.0.0_R8.and.jwire(n).gt.jvvlo(i))  jvvlo(i)       &  
     & =jwire(n)
        if (zwire(n).ge.0.0_R8.and.jwire(n).lt.jvvhi(i))  jvvhi(i)       &  
     & =jwire(n)
                                       endif
   40   continue
...
!.....define masking array
      do 200 i=2,nxp
         if(jvvlo(i).eq.2 .or. jvvhi(i).eq.nzp) then
            jvvlo(i) = nzp
            jvvhi(i) = 2
         endif
      do 200 j=2,nzp
