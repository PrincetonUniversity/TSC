...
!
      if(.not.allocated)allocate(xi_bdy(npsi))
      if(.not.allocated)allocate(rmajmp(2*npsi-1))
      if(.not.allocated)allocate(bmidp(2*npsi-1))
      if(.not.allocated)allocate(ner(npsi))

      xi_bdy(1) = 0.0_R8
      do i = 1, npsit-1
      xi_bdy(i+1) = xi_bdy(i) + 1.0_R8/float(npsi-1)
      enddo
      xi = 0._R8
      do i = 1, npsit-1
...
!                              + xi_bdy(i+1)*rminor
      ...
      call ne_from_trdatbuf(times,xi_bdy,rmajmp,bmidp,ner,npsit-1)
      do i = 1, npsit-1
      ...
      enddo
      rval = rval*1.0_R8e+19*usdd
      rpval = rpval*1.0_R8e+19*usdd
      if( psv .le. psimin ) then
      rval = ner(1)*1.0_R8e+19*usdd
      rpval = 0._R8
      endif
! --------------------------------------------------------------
